...
- **SSH authorized_keys**: ttyforce writes per-user SSH keys to `/town-os/ssh/authorized_keys/{username}` (dir 755, files 644) via `--ssh-user root,status`. sshd is configured at image time with `AuthorizedKeysFile /town-os/ssh/authorized_keys/%u` — this MUST only point at the `/town-os` directory structure, never `.ssh/authorized_keys` or any other path. The persistent btrfs is the single source of truth for SSH keys. Password authentication remains enabled regardless of whether keys are present. No `Match exec` or boot-time sshd config is needed. Keys are deduplicated by ttyforce so re-running the installer won't create duplicates.
