...
- **DNS**: production uses Cloudflare (1.1.1.1) initially; rolodex overwrites `/etc/resolv.conf` with 127.0.0.2 once running. Dev mode (`LOCAL_DNS`) uses 8.8.8.8 and skips rolodex. Rolodex runs with `--net host` and binds to `127.0.0.2:53` (local resolution) and `remote` (external interface, auto-detected by rolodex). It MUST NOT bind to `0.0.0.0` — that would conflict with podman's DNS listener. The `udp_bind`/`tcp_bind` config fields accept either a single string or a YAML list of strings; the special value `remote` tells rolodex to bind to the external interface address.
