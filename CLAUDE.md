...
- **Persistent overlays are mounted by systemd, not the initrd**: The `/etc` and `/var` overlay mounts (btrfs-backed upper dirs) are handled by `town-os-overlays.service`, not the initrd squashfs hook. This avoids stale `$newroot/...` dentry references that occur when overlays are mounted pre-`switch_root`. The service uses correct post-pivot paths (`/.town/sfs/etc`, `/town-os/etc/overlays/root`) and runs before `local-fs.target` so it's in place before networkd/sshd/resolved start. The initrd only mounts `/town-os` (btrfs) and leaves overlay setup to systemd.
