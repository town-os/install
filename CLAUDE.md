...
- **DNS**: production uses Cloudflare (1.1.1.1) initially; rolodex overwrites `/etc/resolv.conf` with 127.0.0.2 once running. Dev mode (`LOCAL_DNS`) uses 8.8.8.8 and skips rolodex. Rolodex runs with `--net host` and binds to exactly `127.0.0.2:53` (local resolution) and the external interface IP (network clients). It MUST NOT bind to `0.0.0.0` — that would conflict with podman's DNS listener. The external IP is detected at boot via `ip route get 1.1.1.1` and written into the default config. The `udp_bind`/`tcp_bind` config fields accept either a single string or a YAML list of strings for multiple bind addresses.
