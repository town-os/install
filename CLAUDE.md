...
- **SSH authorized_keys**: ttyforce writes per-user SSH keys to `/town-os/ssh/authorized_keys/{username}` (dir 755, files 644) via `--ssh-user root,status`. sshd is configured at image time with `AuthorizedKeysFile .ssh/authorized_keys /town-os/ssh/authorized_keys/%u` so it checks both the standard path and the ttyforce-managed path. Password authentication remains enabled regardless of whether keys are present. No `Match exec` or boot-time sshd config is needed — the `AuthorizedKeysFile` directive handles everything. Keys are deduplicated by ttyforce so re-running the installer won't create duplicates.
