...
make image-x86_64 # cross-build an x86_64 image on a non-x86_64 host (EMULATED, slow)
...
- **Native by default; emulated only on opt-in:** with `TARGET_ARCH` unset (or equal to the host arch) the builder container is the host's native architecture (no `--arch`), runs at native CPU speed, and the produced image's architecture equals the host's — it touches no `binfmt_misc` and uses no emulation. Setting `TARGET_ARCH` (via `BUILD_ARCH=<arch>` / `make image-x86_64`) to a non-host arch switches to a **cross-build**: `image-container.sh` registers `qemu-user-static` binfmt handlers and passes `--arch` so the foreign-arch container runs under emulation (slow). See "Native by default; cross-arch is opt-in and emulated" under Architecture Notes. For speed, a different-arch image is still best built natively on a host of that arch.
- **Base image by architecture:** `image-container.sh` picks the base by `TARGET_ARCH` (defaults to `uname -m`) — `docker.io/library/archlinux` on x86_64, the third-party `docker.io/menci/archlinuxarm` on aarch64 (the official Arch image is x86_64-only). It is passed to `Containerfile.build` as the `BASE_IMAGE` build ARG. Override with the `BASE_IMAGE` **environment variable**, e.g. `BASE_IMAGE=docker.io/lopsided/archlinux make image` (it flows through `make` → `image.sh` → `image-container.sh` via the environment; no Makefile wiring needed). The qemu-user-static binfmt image used for cross-builds is overridable via `BINFMT_IMAGE`.
...
- **Native by default; cross-arch is opt-in and emulated**: By default the image architecture equals the build host's (or same-arch builder container's) architecture — x86_64 host → x86_64 image, aarch64 host → aarch64 image — with no cross-compile, no emulation, and no `binfmt_misc`. The default native path MUST stay that way. **Cross-architecture builds are an explicit opt-in** via `TARGET_ARCH` (threaded from the Makefile's `BUILD_ARCH`): set `BUILD_ARCH` to a non-host arch — or use the `make image-x86_64` convenience target — and `make/image-container.sh` registers `qemu-user-static` binfmt handlers (`--reset -p yes`, F/fix-binary flag) and passes `--arch` to `podman build`/`run` so the whole build runs **emulated** in a foreign-arch Arch container. This is **slow** (pacstrap and the Rust ttyforce build run under emulation) and is the one place the build touches host `binfmt_misc` — gated entirely on `TARGET_ARCH != $(uname -m)`. The single `BUILD_ARCH` knob also renames the image file and drives the arch-suffixed image tags (`rc.latest-`/`release-`), so a cross-built image pulls and publishes the correct arch. Inside the emulated container `uname -m` reports the target arch, so `install.sh` keys all arch-specific choices (kernel package, GRUB target, BIOS partition) off `uname -m` unchanged. Producing a different-arch image natively (faster) still means building on a host of that arch.
