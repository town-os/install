...
- **Overlay mount paths must survive pivot_root**: When the initrd sets up overlay mounts for `/etc` and `/var`, the `lowerdir`, `upperdir`, and `workdir` options must use post-pivot_root absolute paths (e.g. `/.town/sfs/etc`, `/town-os/etc/overlays/root`) — NOT `$newroot/...` paths. After `switch_root`, the `/new_root` prefix no longer exists; the kernel keeps stale dentry refs that usually work but can time out under certain conditions.
