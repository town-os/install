...
- **socat TCP relays** forward the service ports host→guest (`LAN_PROXY_PORTS`, default `80 443 2222:22`; ssh deliberately maps to 2222 so the host's own sshd isn't shadowed). Host→guest traffic over the bridge is always permitted, so no NAT/forward-chain games.
...
| `LAN_PROXY_PORTS` | `80 443 2222:22` | TCP port mappings for `make lan-proxy`, space-separated `listen[:guestport]` entries (listen on the host, relay to the guest). Environment variable. |
...
- **Container image pull policy**: Both the systemcontroller and rolodex services MUST use `--pull=always` so that container images are re-pulled on every (re)start. This ensures updates are picked up without manual intervention. **Image tags are arch-suffixed** (`rc.latest-$(uname -m)`, i.e. `rc.latest-x86_64` / `rc.latest-aarch64`): each repository publishes per-arch tags rather than multi-arch manifests, and since builds are always native, the build arch is the right suffix for everything the image pulls. The Makefile composes the suffix into `CONTROLLER_TAG`/`ROLODEX_IMAGE`/`UI_IMAGE`; `install.sh` carries matching defaults (from `$ARCH`) for direct invocation. On **aarch64** builds, `install.sh` additionally injects `--platform=linux/arm64` into both `podman run --pull=always` invocations (a build-time `sed`, aarch64 only) as a loud-failure guard: if an image published under an `-aarch64` tag isn't actually arm64, the pull fails instead of silently fetching an unrunnable amd64 image. x86_64 units are left untouched (podman's host-arch default is already correct).
