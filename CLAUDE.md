...
- **SSH authorized_keys are configured at boot, not image time**: ttyforce writes per-user SSH keys to `/town-os/ssh/authorized_keys/{username}` via `--ssh-user root,status`. At boot, `ssh-setup.sh` (called from `town-os-overlays.service`) checks if any key files exist. If keys exist, it writes `/etc/ssh/sshd_config.d/town-os.conf` with `AuthorizedKeysFile` pointing at `/town-os/ssh/authorized_keys/%u` and a `Match exec` rule disabling password auth for users with keys. If no keys exist, the drop-in is removed and password auth works normally. This must happen at boot (not image build) because the keys don't exist until ttyforce runs, and baking static config into the squashfs would break SSH on fresh installs.
