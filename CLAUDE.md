...
- **socat TCP relays** forward the service ports host→guest (`LAN_PROXY_PORTS`, default `80 443 5309 2222:22`; ssh deliberately maps to 2222 so the host's own sshd isn't shadowed). Host→guest traffic over the bridge is always permitted, so no NAT/forward-chain games.
...
| `LAN_PROXY_PORTS` | `80 443 5309 2222:22` | TCP port mappings for `make lan-proxy`, space-separated `listen[:guestport]` entries (listen on the host, relay to the guest). Environment variable. |
